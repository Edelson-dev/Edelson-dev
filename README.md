<div align="center">
  
# ¡Hola! 👋 Soy Edelson Arias
  
### Desarrollador Full Stack | Arquitectura Escalable 🚀
  
</div>

<br>

<p align="center">
Joven Desarrollador de Software Full Stack con 6 meses de pasantía y 4 meses de trabajo profesional apasionado por crear soluciones a problemas. Especialista en <strong>React/Next.js</strong> y <strong>Node/ASP.NET</strong>. Mi enfoque es construir código limpio y escalable, aplicando activamente patrones de diseño como <strong>DDD</strong> y <strong>CQRS</strong> para soluciones de alto rendimiento.
</p>

---

## 🛠️ Stack Tecnológico & Frameworks

<div align="center">
  <img src="https://skillicons.dev/icons?i=ts,js,react,nextjs,angular,nodejs,express,cs,dotnet,php,laravel,cpp,python" /><br>
  <img src="https://skillicons.dev/icons?i=mysql,postgres,sqlite,tailwind,redux,git,github,docker,aws" />
</div>

<br>

## 💡 Lo que me Define (Innovación y Liderazgo)

### 🏗️ Arquitectura y Diseño de Código
  
- **Domain-Driven Design (DDD):** Aplicación de principios para modelar software complejo basado en el dominio del negocio.
- **CQRS:** Implementación de la separación de Comandos y Consultas para sistemas escalables y auditables.
- **Microservicios:** Experiencia en la comunicación entre servicios utilizando **Event-Bus** y **Mediator**.
  
### 👥 Mentoría y Calidad

Soy un **promotor activo de código limpio y escalable**, con experiencia demostrada en la **mentoría** de compañeros. Mi objetivo es elevar el estándar técnico del equipo, asegurando la adopción de buenas prácticas y la **legibilidad** del código.

<br>

---

## 📊 Mi Actividad en GitHub

<div align="center">
  
<img src="https://github-readme-stats.vercel.app/api?username=[Edelson-dev]&show_icons=true&theme=vue&hide_border=true&count_private=true" alt="GitHub Stats" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=[Edelson-dev]&theme=vue&hide_border=true" alt="GitHub Streak" />
  
</div>

---

## 📫 Conectemos

<p align="center">
  <a href="mailto:edelsonjaelt@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://www.linkedin.com/in/edelson-ariasdev"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="#"><img src="https://img.shields.io/badge/Portafolio-FF5722?style=for-the-badge&logo=web&logoColor=white" /></a>
</p>
